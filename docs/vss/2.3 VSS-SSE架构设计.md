...
sequenceDiagram
    ...
    participant Ch as messageChan (buffer 见 SSE.MessageChanBuffer，默认 256)

    C->>H: GET /events?type=sev_state&...
    H->>H: Header: text/event-stream, Flush
    H->>Ch: make(chan SSEResponse, buf)
...
1. 每个 SSE 连接独占一个 **`messageChan`**，容量为 **`Config.SSE.MessageChanBuffer`**，**≤0 时服务端默认 256**（见 `internal/server/sse.go`）。  
...
- **`messageChan` 容量由 `SSE.MessageChanBuffer` 决定，未配置或 ≤0 时默认 256**。Logic 若在短时内连续发送超过缓冲且 **`handler` 未及时消费**，**发送方会阻塞**。  
- 开发排障与调参说明见：[5.2 开发心得-SSE架构与注意事项](./5.2%20开发心得-SSE架构与注意事项.md)。  
