...
func calcHeader(header *base.RtmpHeader, prevHeader *base.RtmpHeader, out []byte) int {
	...
	if fmt <= 2 {
		if timestamp > maxTimestampInMessageHeader {
			base.BePutUint24(out[index:], maxTimestampInMessageHeader)
		} else {
			base.BePutUint24(out[index:], timestamp)
		}
		index += 3

		if fmt <= 1 {
			base.BePutUint24(out[index:], header.MsgLen)
			index += 3
			out[index] = header.MsgTypeId
			index++

			if fmt == 0 {
				base.LePutUint32(out[index:], uint32(header.MsgStreamId))
				index += 4
			}
		}
	}
	...
	// 设置扩展时间戳
	if timestamp > maxTimestampInMessageHeader {
		base.BePutUint32(out[index:], timestamp)
		index += 4
	}
