...
func (c *HandshakeClientSimple) WriteC0C1(writer io.Writer) error {
	...
	c.buf[0] = version
	base.BePutUint32(c.buf[1:5], uint32(time.Now().UnixNano()))
	base.BePutUint32(c.buf[5:9], 0) // 4字节模式串保持为0，标识是简单模式
	random1528(c.buf[9:])
...
func (s *HandshakeServer) ReadC0C1(reader io.Reader) (err error) {
	...
	base.BePutUint32(s1, uint32(time.Now().UnixNano()))
	random1528(s1[8:])

	if s.isSimpleMode {
		// s1
		base.BePutUint32(s1[4:], 0)
...
func parseChallenge(b []byte, peerKey []byte, key []byte) []byte {
	...
	//	return nil, ErrRtmp
	// }
	ver := base.BeUint32(b[5:])
	if ver == 0 {
		fmt.Printf("handshake simple mode.")
		return nil
	}
	...
	if offs == -1 {
		fmt.Printf("get digest offs failed. roll back to try simple handshake.")
		return nil
	}
	fmt.Printf("handshake complex mode.")
...
func init() {
	...
	hack := []byte(fmt.Sprintf("random buf of rtmp handshake gen by %s", "SkeyeVSS"))
