...
import (
	...
	"skeyevss/core/app/sev/vss/internal/pkg/ms/base"
)
...
func writeSingleChunkHeader(out []byte, csid int, bodyLen int, typeid uint8, streamid int) {
	...
	out[2] = 0
	out[3] = 0
	base.BePutUint24(out[4:], uint32(bodyLen))
	out[7] = typeid
	base.LePutUint32(out[8:], uint32(streamid))
}
...
func (packer *MessagePacker) writeProtocolControlMessage(writer io.Writer, typeid uint8, val int) error {
	...
	// 4
	_ = base.WriteBe(packer.b, uint32(val))
...
func (packer *MessagePacker) WriteConnect(writer io.Writer, appName, tcUrl string, isPush bool) error {
	...
	var flashVer string
	if isPush {
		flashVer = fmt.Sprintf("FMLE/3.0 (compatible; %s)", "SkeyeVSS 4.0")
	} else {
...
func (packer *MessagePacker) writeStreamIsRecorded(writer io.Writer, streamid uint32) error {
	...
	// 6
	_ = base.WriteBe(packer.b, uint16(base.RtmpUserControlRecorded))
	_ = base.WriteBe(packer.b, uint32(streamid))
...
func (packer *MessagePacker) writeStreamBegin(writer io.Writer, streamid uint32) error {
	...
	// 6
	_ = base.WriteBe(packer.b, uint16(base.RtmpUserControlStreamBegin))
	_ = base.WriteBe(packer.b, uint32(streamid))
...
func (packer *MessagePacker) writePingRequest(writer io.Writer, timestamp uint32) error {
	...
	// 6
	_ = base.WriteBe(packer.b, uint16(base.RtmpUserControlPingRequest))
	_ = base.WriteBe(packer.b, timestamp)
...
func (packer *MessagePacker) writeAcknowledgement(writer io.Writer, seqNum uint32) error {
	...
	// 5
	_ = base.WriteBe(packer.b, seqNum)
...
func (packer *MessagePacker) writePingResponse(writer io.Writer, timestamp uint32) error {
	...
	// 6
	_ = base.WriteBe(packer.b, uint16(base.RtmpUserControlPingResponse))
	_ = base.WriteBe(packer.b, timestamp)
...
func (b *Buffer) WriteTo(w io.Writer) (n int64, err error) {
	if nBytes := b.Len(); nBytes > 0 {
		m, e := w.Write(b.Bytes())
		if m > nBytes {
			return n, fmt.Errorf("Buffer.WriteTo: invalid Write count. expected=%d, actual=%d", nBytes, m)
		}
		b.readPos += m
		n = int64(m)
		if e != nil {
...
func (b *Buffer) grow(n int) {
	...
	fmt.Printf("Buffer::grow. need=%d, old len=%d, cap=%d, new len=%d", n, b.Len(), cap(b.core), newLen)
