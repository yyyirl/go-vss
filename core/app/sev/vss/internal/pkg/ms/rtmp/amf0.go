...
func (amf0) WriteNumber(writer io.Writer, val float64) error {
	...
	return base.WriteBe(writer, val)
}
...
func (amf0) WriteString(writer io.Writer, val string) error {
	if len(val) < 65536 {
		...
		if err := base.WriteBe(writer, uint16(len(val))); err != nil {
			return err
		}
	} else {
		...
		if err := base.WriteBe(writer, uint32(len(val))); err != nil {
			return err
		}
	}
...
func (amf0) WriteObject(writer io.Writer, opa ObjectPairArray) error {
	...
	for i := 0; i < len(opa); i++ {
		if err := base.WriteBe(writer, uint16(len(opa[i].Key))); err != nil {
			return err
		}
		...
		default:
			fmt.Printf("unknown value type. i=%d, v=%+v", i, opa[i].Value)
		}
	}
...
func (amf0) ReadNumber(b []byte) (float64, int, error) {
	...
	return base.BeFloat64(b[1:]), 9, nil
}
...
func (amf0) ReadArray(b []byte) (ObjectPairArray, int, error) {
	...
	if len(b)-index >= 3 && bytes.Equal(b[index:index+3], Amf0TypeMarkerArrayEndBytes) {
		...
		fmt.Printf("amf ReadArray without suffix Amf0TypeMarkerObjectEndBytes.")
	}
...
func (amf0) read(b []byte, index int, k string, ops ObjectPairArray) (ObjectPairArray, int, error) {
	...
	case Amf0TypeMarkerUndefined, Amf0TypeMarkerUnsupported:
		...
		index += l
	default:
		err := fmt.Errorf("unknown type. vt=%d, hex=%s, %s", vt, hex.Dump(base.Prefix(b, 4096)), hex.Dump(base.Prefix(b[index:], 4096)))
		return ops, index, err
	}
