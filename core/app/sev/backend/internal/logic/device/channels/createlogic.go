...
func (l *CreateLogic) Create(req *types.RecordReq) (interface{}, *response.HttpErr) {
	// 日志记录
	opt.NewSystemOperationLogs(l.svcCtx.RpcClients).Make(l.ctx, systemOperationLogs.Types[systemOperationLogs.TypeDeviceChannelCreate], req)
	...
	res, err := response.NewRpcToHttpResp[*deviceservice.Response, uint64]().Parse(
		func() (*deviceservice.Response, error) {
			...
			return l.svcCtx.RpcClients.Device.ChannelCreate(l.ctx, &deviceservice.MapReq{Data: data})
		},
	)
