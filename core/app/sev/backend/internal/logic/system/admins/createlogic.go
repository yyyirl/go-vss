...
func (l *CreateLogic) Create(req *types.RecordReq) (interface{}, *response.HttpErr) {
	// 日志记录
	opt.NewSystemOperationLogs(l.svcCtx.RpcClients).Make(l.ctx, systemOperationLogs.Types[systemOperationLogs.TypeAdminCreate], req)
	...
	res, err := response.NewRpcToHttpResp[*backendservice.Response, uint64]().Parse(
		func() (*backendservice.Response, error) {
			...
			return l.svcCtx.RpcClients.Backend.AdminCreate(l.ctx, &backendservice.MapReq{Data: data})
		},
	)
