...
func (l *DeleteLogic) Delete(req *orm.ReqParams) *response.HttpErr {
	// 日志记录
	opt.NewSystemOperationLogs(l.svcCtx.RpcClients).Make(l.ctx, systemOperationLogs.Types[systemOperationLogs.TypeAdminDelete], req)
	...
	_, err := response.NewRpcToHttpResp[*backendservice.Response, bool]().Parse(
		func() (*backendservice.Response, error) {
			...
			return l.svcCtx.RpcClients.Backend.AdminDelete(l.ctx, data)
		},
	)
