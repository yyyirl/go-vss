...
func (l *RowLogic) Row(req *types.IdQuery) (interface{}, *response.HttpErr) {
	res, err := response.NewRpcToHttpResp[*backendservice.Response, *departments.Item]().Parse(
		func() (*backendservice.Response, error) {
			return l.svcCtx.RpcClients.Backend.DepartmentRow(l.ctx, &backendservice.IDReq{ID: uint64(req.Id)})
		},
	)
	if err != nil {
		return nil, err
