...
func (l *RowLogic) Row(req *types.IdQuery) (interface{}, *response.HttpErr) {
	res, err := response.NewRpcToHttpResp[*configservice.Response, *videoProjects.Item]().Parse(
		func() (*configservice.Response, error) {
			return l.svcCtx.RpcClients.Config.VideoProjectRow(l.ctx, &configservice.IDReq{ID: uint64(req.Id)})
		},
	)
	if err != nil {
		return nil, err
