...
func (l *DeleteLogic) Delete(req *orm.ReqParams) *response.HttpErr {
	// 日志记录
	opt.NewSystemOperationLogs(l.svcCtx.RpcClients).Make(l.ctx, systemOperationLogs.Types[systemOperationLogs.TypeVPDelete], req)
	...
	_, err := response.NewRpcToHttpResp[*configservice.Response, bool]().Parse(
		func() (*configservice.Response, error) {
			...
			return l.svcCtx.RpcClients.Config.VideoProjectDelete(l.ctx, data)
		},
	)
