...
func (l *CreateLogic) Create(req *types.RecordReq) (interface{}, *response.HttpErr) {
	// 日志记录
	opt.NewSystemOperationLogs(l.svcCtx.RpcClients).Make(l.ctx, systemOperationLogs.Types[systemOperationLogs.TypeDictCreate], req)

	res, err := response.NewRpcToHttpResp[*configservice.Response, uint64]().Parse(
		func() (*configservice.Response, error) {
			...
			return l.svcCtx.RpcClients.Config.DictionaryCreate(l.ctx, &configservice.MapReq{Data: data})
		},
	)
