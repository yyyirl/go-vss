...
func (l *RowLogic) Row(req *types.IdQuery) (interface{}, *response.HttpErr) {
	res, err := response.NewRpcToHttpResp[*configservice.Response, *dictionaries.Item]().Parse(
		func() (*configservice.Response, error) {
			return l.svcCtx.RpcClients.Config.DictionaryRow(l.ctx, &configservice.IDReq{ID: uint64(req.Id)})
		},
	)
	if err != nil {
		return nil, err
