...
func (l *ListLogic) List(req *orm.ReqParams) (interface{}, *response.HttpErr) {
	res, err := response.NewRpcToHttpResp[*configservice.Response, *response.ListResp[[]*dictionaries.Item]]().Parse(
		func() (*configservice.Response, error) {
			...
			return l.svcCtx.RpcClients.Config.Dictionaries(l.ctx, data)
		},
	)
	if err != nil {
		return nil, err
