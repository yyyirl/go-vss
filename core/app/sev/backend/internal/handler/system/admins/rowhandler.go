...
func RowHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_1_4_4, contextx.GetPermissionIds(ctx)); err != nil {
		...
		resp, err := admins.NewRowLogic(ctx, svcCtx).Row(&req)
		if err != nil {
