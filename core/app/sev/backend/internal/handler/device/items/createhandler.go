...
func CreateHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_3_1_1, contextx.GetPermissionIds(ctx)); err != nil {
		...
		resp, err := items.NewCreateLogic(ctx, svcCtx).Create(&req)
		if err != nil {
