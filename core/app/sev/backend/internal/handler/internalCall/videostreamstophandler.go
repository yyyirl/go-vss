...
func VideoStreamStopHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_7_1_6, contextx.GetPermissionIds(ctx)); err != nil {
		...
		if err := internalCall.NewVideoStreamStopLogic(ctx, svcCtx).VideoStreamStop(req); err != nil {
