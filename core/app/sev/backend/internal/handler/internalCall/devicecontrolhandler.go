...
func DeviceControlHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_7_1_10, contextx.GetPermissionIds(ctx)); err != nil {
		...
		if err := internalCall.NewDeviceControlLogic(ctx, svcCtx).DeviceControl(req, r.URL.Query().Get("t")); err != nil {
