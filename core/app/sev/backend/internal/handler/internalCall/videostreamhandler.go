...
func VideoStreamHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_7_1_5, contextx.GetPermissionIds(ctx)); err != nil {
		...
		resp, err := internalCall.NewVideoStreamLogic(ctx, svcCtx).VideoStream(req)
		if err != nil {
