package setting

import (
	"net/http"

	"skeyevss/core/app/sev/backend/internal/logic/config/setting"
	"skeyevss/core/app/sev/backend/internal/svc"
	"skeyevss/core/common/source/permissions"
	"skeyevss/core/localization"
	"skeyevss/core/pkg/common"
	"skeyevss/core/pkg/contextx"
	"skeyevss/core/pkg/orm"
	"skeyevss/core/pkg/response"
)

func UpdateHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_1_1_1, contextx.GetPermissionIds(ctx)); err != nil {
			response.New().RequestError(ctx, w, response.MakeError(response.NewHttpRespMessage().Err(err), localization.M1006))
			return
		}

		var req orm.ReqParams
		if err := common.Parse(r, &req); err != nil {
			response.New().RequestError(ctx, w, response.MakeError(response.NewHttpRespMessage().Err(err), localization.M0001))
			return
		}

		if err := setting.NewUpdateLogic(r.Context(), svcCtx).Update(&req); err != nil {
			response.New().RequestError(r.Context(), w, err)
			return
		}

		response.New().Success(r.Context(), w, nil)
	}
}
