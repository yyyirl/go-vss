...
func ListHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var ctx = r.Context()
		if err := permissions.New(ctx).Authentication(contextx.GetSuperState(ctx), permissions.P_0_2_3_3, contextx.GetPermissionIds(ctx)); err != nil {
		...
		resp, err := ms.NewListLogic(ctx, svcCtx).List(&req)
		if err != nil {
